...
console.info(process.env.LOG4JS_CONFIG)
...
global.loggerEth = log4js.getLogger('normal');
loggerEth.debug(".............................................................");

loggerEth.debug('   ');
loggerEth.debug('   ', 'NET STATS CLIENT');
loggerEth.debug('   ', 'v' + pjson.version);
loggerEth.debug('   ');
loggerEth.debug('   ');
...
{
	loggerEth.debug('Starting web3 connection');
...
Node.prototype.checkWeb3Connection = function()
{
	...
	{
		if(web3.isConnected()) {
			loggerEth.debug('Web3 connection established');
		...
		else
		{
			...
			{
				loggerEth.error('Web3 connection attempt', chalk.cyan('#' + this._connection_attempts++), 'failed');
				loggerEth.error('Trying again in', chalk.cyan(500 * this._connection_attempts + ' ms'));
			...
			{
				loggerEth.error('Web3 connection failed', chalk.cyan(MAX_CONNECTION_ATTEMPTS), 'times. Aborting...');
			}
		}
	}
}
...
{
	loggerEth.warn("Uninstalling filters and update interval");
	...
	{
		loggerEth.error("Web3 reset error:", err);
	}

	loggerEth.debug("Web3 reconnect attempts started");
...
{
	loggerEth.debug("process.env.WS_SERVER >>>", process.env.WS_SERVER);
	if( !this._socket )
	{
		loggerEth.debug('wsc', 'Starting socket connection');

		socket = new Socket( process.env.WS_SERVER || 'http://localhost:3000' );
		this.setupSockets();
	}
}
...
Node.prototype.setupSockets = function()
{
	...
	{
		loggerEth.debug('wsc', 'The socket connection has been opened.');
		loggerEth.debug('   ', 'Trying to login');
		loggerEth.debug("WS_SECRET >>> ", WS_SECRET);
		socket.emit('hello', {
			id: self.id,
			info: self.info,
			secret: WS_SECRET
	...
	{
		self._socket = true;
		loggerEth.debug('wsc', 'The socket connection has been established.');
	...
	{
		loggerEth.debug('Socket received some data', data);
        socket.emit('data', {
            id: self.id,
            info: self.info,
            secret: WS_SECRET
	...
	{
		loggerEth.debug('his', 'Got history request');
	...
	{
		self._socket = false;
		loggerEth.error('wsc', 'Socket connection end received');
	})
	.on('error', function error(err)
	{
		loggerEth.error('wsc', 'Socket error:', err);
	})
	.on('timeout', function ()
	{
		self._socket = false;
		loggerEth.error('wsc', 'Socket connection timeout');
	})
	.on('close', function ()
	{
		self._socket = false;
		loggerEth.error('wsc', 'Socket connection has been closed');
	})
	.on('offline', function ()
	{
		self._socket = false;
		loggerEth.error('wsc', 'Network connection is offline');
	})
	.on('online', function ()
	{
		self._socket = true;
		loggerEth.debug('wsc', 'Network connection is online');
	})
	.on('reconnect', function ()
	{
		loggerEth.debug('wsc', 'Socket reconnect attempt started');
	})
	.on('reconnect scheduled', function (opts)
	{
		self._socket = false;
		loggerEth.warn('wsc', 'Reconnecting in', opts.scheduled, 'ms');
		loggerEth.warn('wsc', 'This is attempt', opts.attempt, 'out of', opts.retries);
	})
	.on('reconnected', function (opts)
	{
		self._socket = true;
		loggerEth.debug('wsc', 'Socket reconnected successfully after', opts.duration, 'ms');
	...
	{
		self._socket = false;
		loggerEth.error('wsc', 'Socket reconnect atempt took too long:', err.message);
	})
	.on('reconnect failed', function (err, opts)
	{
		self._socket = false;
		loggerEth.error('wsc', 'Socket reconnect failed:', err.message);
	});
}
...
Node.prototype.emit = function(message, payload)
{
	if(this._socket)
	{
		try {
			...
			loggerEth.debug('wsc', 'Socket emited message:', chalk.reset.cyan(message));
			loggerEth.debug('wsc', payload);
		}
		catch (err) {
			loggerEth.error('wsc', 'Socket emit error:', err);
		}
	}
}
...
{
	loggerEth.debug('==>', 'Getting info');
	loggerEth.debug('Got info');
	...
	try {
		...
		loggerEth.debug('Got info');
		loggerEth.debug(this.info);

		return true;
	}
	catch (err) {
		loggerEth.error("Couldn't get version");
	}
...
Node.prototype.validateLatestBlock = function (error, result, timeString)
{
	...
	if( error )
	{
		loggerEth.error("xx>", "getLatestBlock couldn't fetch block...");
		loggerEth.error("xx>", error);
	...
	if(block === false)
	{
		loggerEth.error("xx>", "Got bad block:", chalk.reset.cyan(result));
	...
	{
		loggerEth.warn("==>", "Got same block:", chalk.reset.cyan(block.number));

		if( _.isEqual(JSON.stringify(this.info.block), JSON.stringify(block)) )
			return false;

		loggerEth.debug(this.stats.block);
		loggerEth.debug(block);
		loggerEth.warn("Blocks are different... updating block");
	}

	loggerEth.debug("==>", "Got block:", chalk.reset.red(block.number));
...
Node.prototype.getStats = function(forced)
{
	...
	{
		loggerEth.debug('==>', 'Getting stats')
		loggerEth.debug('   ', 'last update:', chalk.reset.cyan(lastFetchAgo));
		loggerEth.debug('   ', 'forced:', chalk.reset.cyan(forced === true));
		...
		function (err, results)
		{
			...
			if (err) {
				loggerEth.error('xx>', 'getStats error: ', err);
			...
			loggerEth.debug('==>', 'Got getStats results in', chalk.reset.cyan(results.diff, 'ms'));
...
Node.prototype.getPending = function()
{
	...
	if (this._web3)
	{
		loggerEth.debug('==>', 'Getting Pending')

		web3.eth.getBlockTransactionCount('pending', function (err, pending)
		{
			if (err) {
				loggerEth.error('xx>', 'getPending error: ', err);
				return false;
			}
			...
			loggerEth.debug('==>', 'Got', chalk.reset.red(pending) , chalk.reset.bold.green('pending tx'+ (pending === 1 ? '' : 's') + ' in'), chalk.reset.cyan(results.diff, 'ms'));
...
Node.prototype.getHistory = function (range)
{
	...
	loggerEth.debug('=H=', 'his', 'Got history in');
	...
	loggerEth.debug('his', 'Getting history from', chalk.reset.cyan(interv[0]), 'to', chalk.reset.cyan(interv[interv.length - 1]));
	...
	{
		if (err) {
			loggerEth.error('his', 'history fetch failed:', err);
...
Node.prototype.sendBlockUpdate = function()
{
	...
	loggerEth.debug("wsc", "Sending", chalk.reset.red("block"), chalk.bold.white("update"));
	this.emit('block', this.prepareBlock());
}

Node.prototype.sendPendingUpdate = function()
{
	loggerEth.debug("wsc", "Sending pending update");
...
Node.prototype.sendStatsUpdate = function (force)
{
	if( this.changed() || force ) {
		loggerEth.debug("wsc", "Sending", chalk.reset.blue((force ? "forced" : "changed")), chalk.bold.white("update"));
		var stats = this.prepareStats();
		loggerEth.debug(stats);
...
Node.prototype.setWatches = function()
{
	...
	web3.eth.isSyncing(function(error, sync) {
		if(!error) {
			if(sync === true) {
				web3.reset(true);
				loggerEth.debug("SYNC STARTED:", sync);
			} else if(sync) {
				...
				loggerEth.debug("SYNC UPDATE:", sync);
			} else {
				loggerEth.debug("SYNC STOPPED:", sync);
				self.stats.syncing = false;
				self.setFilters();
			}
		} else {
			self.stats.syncing = false;
			self.setFilters();
			loggerEth.error("SYNC ERROR", error);
		}
	});
}
...
Node.prototype.setFilters = function()
{
	...
	{
		loggerEth.debug("Finished processing", 'latest', 'queue');

		self.getPending();
	}

	this._debouncedChain = debounce(function(hash) {
		loggerEth.debug('>>>', 'Debounced');
	...
	try {
		...
		this.chainFilter.watch( function (err, hash)
		{
			...
			loggerEth.debug('>>>', 'Chain Filter triggered: ', chalk.reset.red(hash), '- last trigger:', chalk.reset.cyan(time));
		...
		loggerEth.debug("Installed chain filter");
	}
	catch (err)
	{
		this.chainFilter = false;

		loggerEth.error("Couldn't set up chain filter");
		loggerEth.error(err);
	}
	...
	try {
		...
		this.pendingFilter.watch( function (err, hash)
		{
			...
			loggerEth.debug('>>>', 'Pending Filter triggered:', chalk.reset.red(hash), '- last trigger:', chalk.reset.cyan(time));
		...
		loggerEth.debug("Installed pending filter");
	}
	catch (err)
	{
		this.pendingFilter = false;

		loggerEth.error("Couldn't set up pending filter");
		loggerEth.error(err);
	}
}
